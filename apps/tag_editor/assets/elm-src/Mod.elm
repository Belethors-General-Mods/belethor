module Mod exposing (Mod, Msg(..), decoder, default, encode, tag2option, update, view)
...
update msg oldMod =
    case msg of
        ...
        Pub pub ->
            { oldMod | published = pub }

        CFile fileUrl ->
            { oldMod | customFile = fileUrl }

        SSE sseMsg ->
            { oldMod | sse = updateModFile oldMod.sse sseMsg }

        Oldrim oldrimMsg ->
            { oldMod | oldrim = updateModFile oldMod.oldrim oldrimMsg }

        Tags tagsMsg ->
            { oldMod | tags = updateTagList oldMod.tags tagsMsg }


updateModFile : Maybe ModFile -> MB.Change (Maybe ModFile) ModFile.Msg -> Maybe ModFile
updateModFile mm updateMsg =
    case updateMsg of
        MB.Replace newModfile ->
            newModfile
                ...
                modfile =
                    case mm of
                        ...
                        Just mf ->
                            mf
            in
            Just (ModFile.update mfMsg modfile)
...
updateTagList tags updateMsg =
    case updateMsg of
        ...
        MB.Edit (MB.Add newItem) ->
            newItem :: tags

        MB.Edit (MB.Remove removeMe) ->
            Utils.List.remove removeMe tags
...
viewModFile attrs name mm cap =
    case mm of
        ...
        Nothing ->
            viewCreateModFile name (Just ModFile.default) (cap << MB.Replace)
